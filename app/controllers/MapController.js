...
m_require('app/models/Station.js');

MobileGas.MapController = M.Controller.extend({
    ...
    mapInit: function() {
        var map = M.ViewManager.getView('locationSearchView', 'map');
        map.initMap({
            callbacks: {
                success: {
                    target: this,
    ...
    onSuccess: function(location) {
        ...
        M.ViewManager.getView('locationSearchView', 'map').updateMap({
        ...
        MobileGas.RequestController.getStationList(this.location);
    },
    ...
    //Hier werden TankstellenMarker gesetzt
    setMarkers: function(stations) {
        var map = M.ViewManager.getView('locationSearchView', 'map');
    ...
    markerClicked: function(marker) {
        console.log(marker);
        if(marker) {
            this.set('station',marker.data);
            MobileGas.LocationSearchDetailController.set('stationId', marker.data.record.id);
            console.log(MobileGas.LocationSearchDetailController.stationId);
            MobileGas.RequestController.getStationFuels(marker.data.record.id);
        }
    }
