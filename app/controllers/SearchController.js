...
MobileFuel.SearchController = M.Controller.extend({
    ...
    init: function(isFirstLoad) {
        if (isFirstLoad) {
            ...
            //this.parseXML(xml);
            MobileFuel.RequestController.getCookie();
    ...
    aggregateSearchInformation:function() {
        ...
        if (this.checkAggregation() == 0) {
            ...
            var brands = MobileFuel.BrandViewController.getSelection();
            var fuels = MobileFuel.FuelViewController.getSelection();
            var radius = MobileFuel.RadiusViewController.getSelection();
            var displayMode = MobileFuel.DisplayModeViewController.getSelection();
...
//BEi Fehler Request verhindern und auf fehlerhafte auswahl hinweisen
        ...
        /*var searchRequest = MobileFuel.SearchRequestModel.createRecord({
         brands:MobileFuel.BrandViewController.getSelection(),
         fuels:MobileFuel.FuelViewController.getSelection(),
         radius:MobileFuel.RadiusViewController.getSelection(),
         displayMode: MobileFuel.DisplayModeViewController.getSelection()
         });*/
...
//call Request Method with parameters: brands fuels radius
    ...
    checkAggregation:function() {
        ...
        if (MobileFuel.BrandViewController.getSelection() == -1) {
            MobileFuel.BrandViewController.alertMethod();
            flag = -1;
        }
        if (MobileFuel.FuelViewController.getSelection() == -1) {
            MobileFuel.FuelViewController.alertMethod();
            flag = -1;
        }
        if (MobileFuel.ModeViewController.getSelection() == -1) {
            MobileFuel.ModeViewController.alertMethod();
            flag = -1;
        }
