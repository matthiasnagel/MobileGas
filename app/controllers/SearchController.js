...
MobileGas.SearchController = M.Controller.extend({
    ...
    init: function(isFirstLoad) {
        if (isFirstLoad) {
            ...
            //this.parseXML(xml);
            MobileGas.RequestController.getCookie();

            MobileGas.BrandViewController.init();
            MobileGas.FuelViewController.init();
    ...
    aggregateSearchInformation:function() {
        ...
        if (this.checkAggregation() == 0) {
            ...
            var brands = MobileGas.BrandViewController.getSelection();
            var fuels = MobileGas.FuelViewController.getSelection();
            var radius = MobileGas.RadiusViewController.getSelection();
            var displayMode = MobileGas.DisplayModeViewController.getSelection();
            var mode = MobileGas.ModeViewController.getSelection();


            var searchRequest = MobileGas.SearchRequestModel.createRecord({
                brands:brands,
                fuels:fuels,
                radius:radius,
            ...
            brands = MobileGas.BrandViewController.getSelectionNames();
            fuels = MobileGas.FuelViewController.getSelectionNames();

            var searchRequestSave = MobileGas.SearchRequestModel.createRecord({
                brands:brands,
                fuels:fuels,
                radius:radius,
                displayMode: displayMode,
            ...
            MobileGas.RequestController.searchStationList(searchRequest);
        }
        else {
            //aggregation error
            console.log('Error');
...
//BEi Fehler Request verhindern und auf fehlerhafte auswahl hinweisen
        ...
        /*var searchRequest = MobileGas.SearchRequestModel.createRecord({
         brands:MobileGas.BrandViewController.getSelection(),
         fuels:MobileGas.FuelViewController.getSelection(),
         radius:MobileGas.RadiusViewController.getSelection(),
         displayMode: MobileGas.DisplayModeViewController.getSelection()
         });*/
...
//call Request Method with parameters: brands fuels radius
    ...
    checkAggregation:function() {
        ...
        if (MobileGas.BrandViewController.getSelection() == -1) {
            MobileGas.BrandViewController.alertMethod();
            flag = -1;
        }
        if (MobileGas.FuelViewController.getSelection() == -1) {
            MobileGas.FuelViewController.alertMethod();
            flag = -1;
        }
        if (MobileGas.ModeViewController.getSelection() == -1) {
            MobileGas.ModeViewController.alertMethod();
            flag = -1;
        }
    ...
    handleStationList:function(data) {
                    ...
                    $.each(data, function(key, val) {
                        if (val.id.toString() == that.attr('brand')) {
                            stations.push(MobileGas.Station.createRecord({
                                ...
                                fuel:MobileGas.Fuel.createRecord({
        ...
        //Methode muss auf success warten - robin
            if (stations.length == 0) {
                        ...
                        MobileGas.NavigationController.switchToResultsView();
                    }
        }
